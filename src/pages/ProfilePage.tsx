            ...
            <div className="bg-gradient-to-br from-brand-50 to-sky-50 rounded-2xl border border-brand-200 p-5 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between p-2 bg-white rounded-lg">
                  ...
                  <span className="font-bold text-red-600">{currentUser.bloodGroup || N}</span>
                </div>
                <div className="flex items-center justify-between p-2 bg-white rounded-lg">
                  <span className="text-slate-600">Age:</span>
                  <span className="font-bold text-slate-900">{currentUser.age || N} years</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm text-slate-900 font-medium">{currentUser.phone || N}</p>
                )}
              </div>
              ...
              {currentUser.role === 'hospital_staff' && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-900 font-medium">{currentUser.department || N}</p>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-600 block mb-1.5">Specialization</label>
                    <p className="text-sm text-slate-900 font-medium">{currentUser.specialization || N}</p>
                  </div>
                </>
              )}
              ...
              {currentUser.role === 'admin' && (
                <div>
                  ...
                  <p className="text-sm text-slate-900 font-mono font-medium">{currentUser.badgeNumber || N}</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-sm text-slate-900 font-medium">{currentUser.emergencyContact || N}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-sm text-slate-900">{currentUser.medicalInfo || N}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-900 font-medium flex items-center gap-2">
                    ...
                    {currentUser.hospitalName || N}
                  </p>
                </div>

                <div>
                  <label className="text-xs font-semibold text-slate-600 block mb-1.5">Experience</label>
                  <p className="text-sm text-slate-900 font-medium">{currentUser.experienceYears || N} years</p>
                </div>

                <div className="sm:col-span-2">
                  <label className="text-xs font-semibold text-slate-600 block mb-1.5">Hospital ID</label>
                  <p className="text-sm text-slate-900 font-mono font-medium">{currentUser.hospitalId || N}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 bg-slate-50 rounded-lg">
                  ...
                  <span className="text-slate-900 font-medium">{currentUser.location || N}</span>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg">
